...
fn start_health_check_server(is_sentinel: bool) {
    ...
    let server = Server::new(addr, move |request| {
        router!(request,
            (GET) (/liveness) => { handle_health_check_with_config(is_sentinel, check_handler_liveness, &redis_pool, "liveness") },
            (GET) (/readiness) => { handle_health_check_with_config(is_sentinel, check_handler_readiness, &redis_pool, "readiness") },
            (GET) (/startup) => { handle_health_check_with_config(is_sentinel, |_, _| Ok(true), &redis_pool, "startup") },
...
fn get_health_config_from_configmap() -> Result<HealthConfig, Box<dyn std::error::Error>> {
    ...
    rt.block_on(async {
        let config_name = env::var("HEALTH_CONFIG_NAME").unwrap_or_else(|_| "health-config".to_string());
...
fn parse_health_config_from_data(data: BTreeMap<String, String>) -> Result<HealthConfig, Box<dyn std::error::Error>> {
