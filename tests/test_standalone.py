...
SERVICE_ID = "sp-JvkxkPhinN"
SUBSCRIPTION_ID = "sub-kFDyC8pT5i"
...
def _get_instance_connection_data(token, instance_id):
    ...
    # Select the first resource with clusterEndpoint
    resource = None
    for key in resources_keys:
        if "clusterEndpoint" in resources[key]:
            resource = resources[key]
            break

    if resource is None:
        print("No resource with clusterEndpoint found")
        return
