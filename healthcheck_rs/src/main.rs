    ...
    F: Fn(bool, &redis::Client) -> Result<bool, redis::RedisError>,
{
    ...
    match check_fn(is_sentinel, redis_client) {
...
fn check_handler_liveness(is_sentinel: bool, redis_client: &redis::Client) -> Result<bool, redis::RedisError> {
    let connection = redis_client.get_connection();

    match connection {
        Ok(mut conn) => {
            if is_sentinel {
                return check_sentinel(&mut conn);
            }
