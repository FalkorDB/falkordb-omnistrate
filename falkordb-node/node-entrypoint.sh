...
REPLACE_SENTINEL_ACL_CONF=${REPLACE_SENTINEL_ACL_CONF:-0}
...
SENTINEL_ACL_CONF=$DATA_DIR/sentinel-acl.conf
...
if [ ! -f $SENTINEL_ACL_CONF ] || [ "$REPLACE_SENTINEL_ACL_CONF" -eq "1" ]; then
  echo "Copying sentinel-acl.conf from /falkordb"
  cp /falkordb/sentinel-acl.conf $SENTINEL_ACL_CONF
fi
...
if [[ "$RUN_SENTINEL" -eq "1" ]] && ([[ "$NODE_INDEX" == "0" || "$NODE_INDEX" == "1" ]]); then
  ...
  sed -i "s|\$SENTINEL_ACL_FILE|$SENTINEL_ACL_CONF|g" $SENTINEL_CONF_FILE
  ...
  sed -i "s/\$ADMIN_PASSWORD/#$ADMIN_PASSWORD_HASH/g" $SENTINEL_ACL_CONF
