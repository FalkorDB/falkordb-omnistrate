...
# Delete paid (non-Free) production instances that have been continuously STOPPED
# for at least AGE_THRESHOLD_DAYS days. Free tier instances are handled separately
...
AGE_THRESHOLD_DAYS="${AGE_THRESHOLD_DAYS:-14}"
...
# List all stopped production instances. Free tier instances are skipped in the
# loop below since they are handled by a separate job.
instances=$(omnistrate-ctl instance list -f service:FalkorDB,environment:Prod,status:STOPPED -o json | jq -r '.[].instance_id')
...
for instance in $instances; do
  ...
  if [ "$diff" -ge "$AGE_THRESHOLD_DAYS" ] && [ "$status" = "STOPPED" ]; then
    if [ "$DRY_RUN" = "true" ]; then
      echo "[DRY_RUN] Would delete stopped instance: $instance (plan: $plan, stopped $diff days ago - $last_modified)"
      continue
    fi
    ...
    echo "Deleting unused stopped instance: $instance (plan: $plan, stopped $diff days ago - $last_modified)"
    ...
    echo "Instance $instance (plan: $plan) was last modified $diff days ago with status $status. Skipping."
  fi
done
