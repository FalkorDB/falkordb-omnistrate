// PRE-FIX version of the healthcheck binary.
//
// The two intentional bugs that cause the memory leak are preserved here so
// they can be deployed side-by-side with the fixed binary for comparison:
//
//   1. new_current_thread runtime — hyper/kube background cleanup tasks only
//      run *inside* block_on.  Between probe invocations they queue up and are
//      never drained, so memory climbs monotonically.
//
//   2. Regex compiled on every probe call — minor but adds steady per-call
//      allocation pressure.
...
// BUG 1: current_thread runtime — background tasks never drain between probes.
...
fn get_health_config_from_configmap() -> Result<HealthConfig, Box<dyn std::error::Error>> {
    ...
    let cfg = TOKIO_RT.block_on(async {
        ...
        match tokio::time::timeout(Duration::from_secs(1), api.get(&name)).await {
            ...
            Ok(Err(err)) => {
                eprintln!("healthcheck: k8s error: {}", err);
                Ok(HealthConfig::default())
            }
...
fn get_namespace() -> Result<String, Box<dyn std::error::Error>> {
    match std::fs::read_to_string("/var/run/secrets/kubernetes.io/serviceaccount/namespace") {
        ...
        Err(_) => env::var("NAMESPACE").or_else(|_| Ok("default".to_string())),
    }
}
...
fn parse_health_config_from_data(
    ...
    if let Some(v) = data.get("skip_all") {
        config.skip_all = Some(v.trim().to_lowercase() == "true");
    }
    if let Some(v) = data.get("skip_liveness") {
        config.skip_liveness = Some(v.trim().to_lowercase() == "true");
    }
    if let Some(v) = data.get("skip_readiness") {
        config.skip_readiness = Some(v.trim().to_lowercase() == "true");
    }
    if let Some(v) = data.get("skip_startup") {
        config.skip_startup = Some(v.trim().to_lowercase() == "true");
    }

    if let Some(json_config) = data.get("config.json") {
        if let Ok(parsed) = serde_json::from_str::<HealthConfig>(json_config) {
            return Ok(parsed);
        }
    }
...
fn check_node_readiness(
    ...
    // BUG 2: regex compiled on every call instead of once.
...
fn get_redis_role(db_info: &str) -> Result<&str, redis::RedisError> {
    // BUG 2: compiles the regex on every single call.
