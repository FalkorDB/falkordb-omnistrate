...
if [ -n "$RUN_METRICS" ] && [ "$RUN_METRICS" -eq "$RUN_METRICS" ] 2>/dev/null && [ "$RUN_METRICS" -eq 1 ]; then
  ...
  extra_args=$(if [ "$is_node" -eq "1" ]; then echo "$is_cluster --is-falkordb --include-falkordb-graph-memory --exclude-falkordb-graph-memory-attrs -slowlog-history-enabled $aof_metric_export"; else echo ""; fi)
