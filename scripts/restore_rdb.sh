...
NAMESPACE=${NAMESPACE:-'instance-ugnmfs6jq'}
...
wipe_all() {
  ...
  exec_node_1 rm -rf /data/*
  exec_sentinel_0 rm -rf /data/*
}
...
slaveof=$(exec_node_1 cat /data/node.conf | grep "slaveof" | awk '{print $2 "\t" $3}')
exec_node_1 redis-cli --no-auth-warning -a $ADMIN_PASSWORD slaveof "no one"
...
exec_node_0 redis-cli --no-auth-warning -a $ADMIN_PASSWORD config set save ""
...
exec_node_0 cp /data/dump.rdb /data/dump.old.rdb
exec_node_0 cp /data/new.rdb /data/dump.rdb
exec_node_1 cp /data/new.rdb /data/dump.rdb
exec_node_1 cp /data/dump.rdb /data/dump.old.rdb
...
echo "$(date): Master has finished loading with size $(exec_node_0 redis-cli --no-auth-warning -a $ADMIN_PASSWORD info | grep 'used_memory_human')"
...
exec_node_1 redis-cli --no-auth-warning -a $ADMIN_PASSWORD slaveof $slaveof
